...
export function IntroHero() {
  ...
  useGSAP(
    () => {
      ...
      const firstLetters = firstNameRef.current.filter(
        Boolean
      ) as HTMLSpanElement[];
      const lastLetters = lastNameRef.current.filter(
        Boolean
      ) as HTMLSpanElement[];
      const meta = metaRef.current.filter(Boolean) as HTMLLIElement[];
      const ctas = ctaRef.current.filter(Boolean) as HTMLAnchorElement[];
      ...
      gsap.set([membraneOuter, membraneInner, membraneSheen], {
        opacity: 0,
        scale: 1.02,
      });
      ...
      timeline.to(
        statement,
        { y: 0, opacity: 1, filter: "blur(0px)", duration: 0.34 },
        "-=0.24"
      );

      timeline.to(
        paragraph,
        { y: 0, opacity: 1, filter: "blur(0px)", duration: 0.36 },
        "-=0.22"
      );
      ...
      if (supportsPointerParallax) {
        const cellX = gsap.quickTo(cell, "x", {
          duration: 0.5,
          ease: "power3.out",
        });
        const cellY = gsap.quickTo(cell, "y", {
          duration: 0.5,
          ease: "power3.out",
        });
        const cellRotate = gsap.quickTo(cell, "rotate", {
          duration: 0.6,
          ease: "power3.out",
        });
        const outerX = gsap.quickTo(membraneOuter, "x", {
          duration: 0.65,
          ease: "power3.out",
        });
        const outerY = gsap.quickTo(membraneOuter, "y", {
          duration: 0.65,
          ease: "power3.out",
        });
        const innerX = gsap.quickTo(membraneInner, "x", {
          duration: 0.58,
          ease: "power3.out",
        });
        const innerY = gsap.quickTo(membraneInner, "y", {
          duration: 0.58,
          ease: "power3.out",
        });
        const sheenX = gsap.quickTo(membraneSheen, "x", {
          duration: 0.72,
          ease: "power3.out",
        });
        const sheenY = gsap.quickTo(membraneSheen, "y", {
          duration: 0.72,
          ease: "power3.out",
        });
        ...
        return () => {
          ...
          loopingTweens.forEach((tween) => tween.kill());
        };
      }
      ...
      <div className="mx-auto flex w-full max-w-[76rem] items-center justify-center px-5 sm:px-10 lg:px-0">
        <div className="hero-scene-frame w-full">
          <div className="hero-scene grid w-full items-center gap-10 md:grid-cols-[minmax(0,1fr)_minmax(21rem,25rem)] md:gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(30rem,36rem)] xl:grid-cols-[minmax(0,1fr)_minmax(34rem,40rem)] xl:gap-10">
            ...
            <div className="hero-visual-shell relative mx-auto flex h-[28rem] w-full max-w-[21.5rem] items-end justify-center md:h-[31rem] md:max-w-[24rem] lg:mx-0 lg:ml-auto lg:h-[40rem] lg:max-w-[36rem] xl:h-[45rem] xl:max-w-[40rem]">
              <div
                ref={visualRef}
                className="hero-visual relative h-full w-full will-change-transform"
              >
                <div
                  ref={cellRef}
                  className="hero-cell absolute right-[2%] top-[4%] z-0 h-[88%] w-[88%] rounded-[46%_54%_52%_48%/42%_38%_62%_58%]"
                >
                  <div
                ...
                <div
                  ref={portraitRef}
                  className="hero-portrait absolute inset-y-[8%] right-[7%] z-10 flex w-[78%] items-end justify-center overflow-hidden rounded-[44%_56%_50%_50%/40%_44%_56%_60%]"
                >
                  <Image
                    src="/profile.png"
                    ...
                    height={1220}
                    priority
                    sizes="(min-width: 1280px) 36rem, (min-width: 1024px) 32rem, (min-width: 768px) 24rem, 78vw"
                    className="relative z-10 h-auto max-h-[25rem] w-auto max-w-none object-contain drop-shadow-[0_18px_40px_rgba(0,0,0,0.35)] sm:max-h-[32rem] xl:max-h-[43rem]"
                  />
                </div>
              </div>
            </div>
